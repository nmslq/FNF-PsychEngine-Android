...
class NoteSplashDebugState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(selecArr != null)
		{
			...
			if(FlxG.keys.pressed.SHIFT #if android || virtualPad.buttonD.justPressed #end)
			{
				movex *= 10;
				movey *= 10;
			}
		...
		if (FlxG.keys.justPressed.SPACE #if android || virtualPad.buttonB.justPressed #end) changeAnim();
		else if (FlxG.keys.justPressed.S #if android || virtualPad.buttonDown.justPressed #end) changeAnim(-1);
		else if (FlxG.keys.justPressed.W #if android || virtualPad.buttonUp.justPressed #end) changeAnim(1);
