...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthStrips.visible = !ClientPrefs.data.hideHud;
		healthStrips.alpha = ClientPrefs.data.healthBarAlpha;
		healthStrips.color = FlxColor.BLACK;
		healthStrips.blend = MULTIPLY;
		healthStrips.x = healthBar.x - 5.9;
		add(healthStrips);
		if(ClientPrefs.data.downScroll) healthStrips.y = 0.11 * FlxG.height;
	...
	function startCharacterScripts(name:String)
	{
		...
		{
			scriptFile = SUtil.getStorageDirectory() + Paths.getPreloadPath(scriptFile);
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if (scoreDisplays.ratingPercent)
			tempScore += (ratingName != '?' ? ' (${CoolUtil.floorDecimal(ratingPercent * 100, 2)}%)' : '');
	...
	override public function update(elapsed:Float)
	{
		...
		else iconP1.animation.curAnim.curFrame = (healthBar.percent < 20) ? 1 : 0;
		...
		else iconP2.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : 0;
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							(Conductor.songPosition - daNote.strumTime > noteKillOffset)
							{
	...
	public function startHScriptsNamed(scriptFile:String)
	{
		...
		if(!FileSystem.exists(scriptToLoad))
			scriptToLoad = SUtil.getStorageDirectory() + Paths.getPreloadPath(scriptFile);
